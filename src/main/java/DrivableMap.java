...
class DrivableMap {
    ...
    public boolean hasFasterThan(int speed){
        for (Drivable elem: this.drivable_map.values()){
            if (elem.getMaxSpeed() >= speed){
                return true;
            }
        }
        return false;
    ...
    public List<Tradable> getTradable(){
        List<Tradable> myList = new ArrayList<>();
        for( Drivable elem: this.drivable_map.values()){
            if (elem instanceof Tradable){
                myList.add((Tradable)elem);
            }
        }
        return myList;
    }
